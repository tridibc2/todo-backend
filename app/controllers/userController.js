    ...
    return new Promise((resolve, reject) => {
      if (req.body.email) {
        ...
        UserModel.findOne({ email: req.body.email }, (err, userDetails) => {
          /* error handler if the User not found */
          if (err) {
            console.log(err);
            logger.error(
            ...
            /* generate error and api response message */
            ...
            /* generate error and api response message */
            logger.error("No User Found", "userController: findUser()", 6);
            let apiResponse = response.generate(
              true,
  ...
  let validatePassword = (retrievedUserDetails) => {
    ...
    return new Promise((resolve, reject) => {
      passwordLib.comparePassword(
        ...
        (err, isMatch) => {
          /* error handler for password field */
          if (err) {
            console.log(err);
...
let requested = (req, res) => {
  ...
  UserModel.update({ userId: req.body.request }, options).exec(
    (err, result) => {
      if (err) {
        console.log(err);
        logger.error(err.message, "userController: Database error", 10);
      ...
      UserModel.update({ userId: req.params.userId }, options).exec(
        (err, result) => {
          if (err) {
            ...
            let apiResponse = response.generate(
              true,
              "Failed To Add User To Friends Array",
              500,
              null
            );
            reject(apiResponse);
      ...
      UserModel.update({ userId: req.body.request }, options).exec(
        (err, result) => {
          if (err) {
            ...
            let apiResponse = response.generate(
              true,
              "Failed To Add To Friends Array",
              500,
              null
            );
            reject(apiResponse);
            ...
            let apiResponse = response.generate(
              false,
              "Successfully Added To Friends Array",
              200,
              null
            );
            resolve(apiResponse);
      ...
      UserModel.update({ userId: req.params.userId }, options).exec(
        (err, result) => {
          if (err) {
            ...
            let apiResponse = response.generate(
              true,
              "Failed To Remove From Requested",
              500,
              null
            );
            reject(apiResponse);
            ...
            let apiResponse = response.generate(
              false,
              "Successfully Removed From Requested",
              200,
              result
            );
            resolve(apiResponse);
      ...
      UserModel.update({ userId: req.body.request }, options).exec(
        (err, result) => {
          if (err) {
            ...
            let apiResponse = response.generate(
              false,
              "Successfully Removed From Request",
              200,
              result
            );
            resolve(apiResponse);
...
let signUpFunction = (req, res) => {
  ...
  validateUserInput(req, res)
    ...
    .then((resolve) => {
      ...
      logger.info("user created@", "userController: signUpFunction");
...
let forgotPasswordFunction = (req, res) => {
  let validateUserInputForForgotPassword = () => {
    return new Promise((resolve, reject) => {
      if (req.body.email) {
        ...
        logger.error(
          "Field Missing During User Creation",
          "userController: Forgot password controller",
          5
        );
        let apiResponse = response.generate(
          true,
  ...
  let findUser = () => {
    new Promise((resolve, reject) => {
      UserModel.findOne({ email: req.body.email }).exec((err, validEmail) => {
        if (err) {
          logger.error(
            err.message,
            "userController: Forgot password controller",
  ...
  validateUserInputForForgotPassword(req, res)
    ...
    .then((resolve) => {
      let apiResponse = response.generate(
        false,
        "Sending recovery mail. Please check your email.",
        200,
        null
      ); // null because dont want to show data to stranger
      res.send(apiResponse);
    })
    .catch((err) => {
      console.log(err);
...
let sendInvite = (req, res) => {
  UserModel.findOne({ userId: req.query.userId }).exec((err, validEmail) => {
    if (err) {
      ...
      mail.invitationEmail(
        validEmail.userId,
        validEmail.firstName,
        req.query.email
      ); //code for sending email to reset password
...
let addInvitedFriend = (req, res) => {
  ...
  UserModel.update({ userId: req.query.userId }, options).exec(
    (err, result) => {
      if (err) {
        ...
        UserModel.update(
          { userId: req.query.inviteId },
          { $push: { friends: req.query.userId } }
        ).exec((err, result) => {
          if (err) {
            logger.error(
              err.message,
              "userController: Adding invited friend ",
              10
            );
            let apiResponse = response.generate(
              true,
